...
- **Resolved during self-review:** checked `app/page.tsx` section order — `DemoSection` (immediately before `ContactSection`) is already `bg-[var(--bg-navy)]`. Switching `ContactSection` to navy too would stack two dark sections back to back with no rhythm break. **Decision: keep `bg-[var(--bg-subtle)]` (light)** and use a light-styled elevated card for the right column instead of the navy/grain pattern.
